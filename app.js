...
app.get("/test", (req, resp) => {
  ...
  request.post(
    ...
    function(err, res, body) {
      var cookie_string = j.getCookieString(url_login),
      ...
      request
        .get(
          ...
          (e, r, b) => {
            var cookie_string2 = j2.getCookieString(url_download),
			...
			// works, uncomment when necessary
		...
		// filename doesnt come up at the right time, consider using promises
        // .pipe(fs.createWriteStream(path.join(__dirname, dir, filename)))
        .pipe(fs.createWriteStream(path.join("./test/", filename + "a.srt")));
...
app.get("/:show?/:season?/:episode?/:language?", function(req, res, next) {
  ...
  if (typeof localStorage.getItem("shows") !== "string") {
    ...
    var cache = {
      shows: {
        timestamp: moment(
          JSON.parse(localStorage.getItem("shows")).timestamp
        ).format(),
  ...
  if (
    ...
    let request_shows = request(opts, (err, resp, body) => {
      if (!err) {
        ...
        var html = body.replace(/<img\b[^>]*>/gi, ""),
          $ = cheerio.load(html, {
  ...
  if (params.show) {
    ...
    request(options, function(error, response, body) {
      if (!error) {
        ...
        requestData.seasons = $("#qsiSeason option")
          .map(function(i, el) {
            return {
              id: parseInt($(el).val()),
              name: $(el).text()
  ...
  if (params.season) {
    ...
    request(options, function(error, response, body) {
      if (!error) {
        ...
        requestData.episodes = $("#qsiEp option")
          .map(function(i, el) {
            return {
              id: $(el)
                .val()
  ...
  if (params.episode) {
    ...
    request(options, function(error, response, body) {
      if (!error) {
        ...
        requestData.episode_subtitles = rows
          .map((i, el) => {
            return {
              version: $(el)
                .find("td.c")
